...
class MainModel(nn.Module):
    def __init__(self, num_labels=2):
        ...
        self.dna2vec_processor = DNA2VecProcessor(num_labels=num_labels)
        self.net = Net()


    def forward(self, x1, x2, x3, x4, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer):
        custom = self.custom_model(x1, x2, x3, x4)
        dna2vec_processed = self.dna2vec_processor(dna2vec_4mer, dna2vec_5mer, dna2vec_6mer)
        combined_features = torch.cat((custom, dna2vec_processed), dim=-1)
        logits = self.net(combined_features)
...
def train_net(device, net, trainloader, valloader, epochs, lr):
    ...
    print("Training....")
    overall_progress = tqdm(total=epochs, desc="Training", position=0, leave=True)
    start_time = time.time()

    for epoch in range(epochs):
        net.train()
        epoch_loss = 0
        for i, (one_hot, chem, eiip, enac, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer, label) in enumerate(trainloader):
            one_hot, chem, eiip, enac, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer, label = [
                feat.to(device) for feat in (one_hot, chem, eiip, enac, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer, label)
            ]
            label = label.float()
            logits = net(one_hot, chem, eiip, enac, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer).squeeze()
        ...
        with torch.no_grad():
            for val_batch in valloader:
                one_hot, chem, eiip, enac, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer, label = [
                    feat.to(device) for feat in val_batch
                ]
                label = label.float()
                logits = net(one_hot, chem, eiip, enac, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer).squeeze()
                loss = criterion(logits, label)
        ...
        overall_progress.set_postfix(train_loss=avg_train_loss, val_loss=avg_val_loss)
    ...
    return net, train_time, train_losses, val_losses
...
def predict(device, model, dataloader):
    ...
    with torch.no_grad():
        for data in dataloader:
            one_hot, chem, eiip, enac, dna2vec_4mer, dna2vec_5mer, dna2vec_6mer, labels = data
            inputs = (one_hot.to(device), chem.to(device), eiip.to(device), enac.to(device),
                      dna2vec_4mer.to(device), dna2vec_5mer.to(device), dna2vec_6mer.to(device))
