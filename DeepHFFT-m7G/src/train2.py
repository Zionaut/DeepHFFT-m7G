...
def create_data_loader(dict_data, indices, shuffle, batch_size=batch_size):
...
def main():
    ...
    print(f"mini_batch_size = {batch_size}")
    ...
    for f, (train_idx, val_idx) in enumerate(skf.split(train_data, [d['label'] for d in train_data])):
        ...
        net = MainModel()
        net, train_time, train_losses, val_losses = train_net(device, net, trainloader, valloader, epochs, lr)
        model_dir = './models'
        ...
        loss_df.to_csv(os.path.join(loss_dir, f'loss_fold{f + 1}.csv'), index=False)
        plt.figure()
        plt.plot(range(1, epochs + 1), train_losses, label='Train Loss')
        plt.plot(range(1, epochs + 1), val_losses, label='Val Loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title(f'Fold {f + 1} Loss Curve')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'loss_curve_fold{f + 1}.png')
        plt.close()
